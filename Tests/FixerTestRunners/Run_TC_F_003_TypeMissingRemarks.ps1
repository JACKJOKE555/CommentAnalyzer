...
Write-Host "=== TC_F_003: 类型缺失备注修复测试 ===" -ForegroundColor Cyan
...
$testFile = Join-Path $testCaseDir "FixerTestCases/TC_F_003_TypeMissingRemarks.cs"
$templateFile = Join-Path $testCaseDir "FixerTestCases/Templates/TC_F_003_TypeMissingRemarks.txt"
...
$fixerArgs = @{
    Mode = "fix"
    SolutionPath = "../../Dropleton.csproj"
    ScriptPaths = @($testFile)
}
if ($DebugType -and $DebugType.Count -gt 0) {
    $fixerArgs["DebugType"] = $DebugType
}
& $commentAnalyzer @fixerArgs
