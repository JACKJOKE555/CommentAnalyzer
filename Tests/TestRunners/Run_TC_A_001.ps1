<#
.SYNOPSIS
    Test case for analyzer rule: PROJECT_TYPE_MISSING_SUMMARY (TC_A_001)
.DESCRIPTION
    This script runs the CommentAnalyzer on a specific test case file and verifies
    that the analyzer correctly identifies types that have a comment block but are
    missing a <summary> tag.
#>

# --- Test Configuration ---
$TestName = "TC_A_001: Type Missing Summary"
$RuleIdToFind = "PROJECT_TYPE_MISSING_SUMMARY"
$ExpectedDetections = 4 # One for class, struct, interface, and enum
...
$TestCasePath = Join-Path $RepoRoot "CustomPackages/CommentAnalyzer/Tests/TestCases/TC_A_001_TypeMissingSummary.cs"
...
$LogFilePath = Join-Path $PSScriptRoot "..\\..\\Logs\\TC_A_001_$(Get-Date -Format 'yyyyMMddHHmmss').log"
