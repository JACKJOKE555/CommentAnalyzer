<#
.SYNOPSIS
    Test case for analyzer rule: PROJECT_MEMBER_MISSING_RETURNS (TC_A_014)
.DESCRIPTION
    验证void方法不应被要求returns标签，所有方法均已注释，无诊断。
#>

# --- Test Configuration ---
$TestName = "TC_A_014: Simple Void Test"
$RuleIdToFind = "PROJECT_MEMBER_MISSING_RETURNS"
$ExpectedDetections = 0
$TestCaseFile = "TC_A_014_SimpleVoidTest.cs"
...
$LogFilePath = Join-Path $PSScriptRoot "..\..\Logs\TC_A_014_$(Get-Date -Format 'yyyyMMddHHmmss').log"
$roslynatorArgs = @(
    "analyze",
    $tempProjectPath,
...
Write-Host "实际检测到 $detections 条 $RuleIdToFind 诊断。"
