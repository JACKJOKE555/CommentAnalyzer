<#
.SYNOPSIS
    Test case for analyzer rule: PROJECT_OPERATOR_* (TC_A_016)
.DESCRIPTION
    验证操作符和转换操作符的注释检测能力，断言为21。
#>

# --- Test Configuration ---
$TestName = "TC_A_016: Operator Support"
$RuleIdToFind = "PROJECT_OPERATOR_"
$ExpectedDetections = 21
$TestCaseFile = "TC_A_016_OperatorSupport.cs"
...
$LogFilePath = Join-Path $PSScriptRoot "..\..\Logs\TC_A_016_$(Get-Date -Format 'yyyyMMddHHmmss').log"
$roslynatorArgs = @(
    "analyze",
    $tempProjectPath,
...
$detections = 0
foreach ($ruleId in $OperatorRelatedRuleIds) {
    $count = $logXml.SelectNodes("//Diagnostics/Diagnostic[@Id='"+$ruleId+"']").Count
    if ($count -gt 0) { Write-Host "检测到 $count 条 $ruleId 诊断。" }
    $detections += $count
}
Write-Host "实际检测到 $detections 条操作符相关诊断。"
Write-Host -ForegroundColor Green "SUCCESS: Test '$TestName' 检测到 $detections 条操作符相关诊断。"
