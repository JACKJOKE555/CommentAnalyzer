<#
.SYNOPSIS
    Test case for analyzer rule: PROJECT_DESTRUCTOR_* (TC_A_017)
.DESCRIPTION
    验证析构函数和索引器的注释检测能力，断言为17。
#>

# --- Test Configuration ---
$TestName = "TC_A_017: Destructor/Indexer Support"
$RuleIdToFind = "PROJECT_DESTRUCTOR_"
$ExpectedDetections = 17
$TestCaseFile = "TC_A_017_DestructorIndexerSupport.cs"
...
$LogFilePath = Join-Path $PSScriptRoot "..\..\Logs\TC_A_017_$(Get-Date -Format 'yyyyMMddHHmmss').log"
$roslynatorArgs = @(
    "analyze",
    $tempProjectPath,
...
$detections = 0
foreach ($ruleId in $DestructorIndexerRuleIds) {
    $count = $logXml.SelectNodes("//Diagnostics/Diagnostic[@Id='"+$ruleId+"']").Count
    if ($count -gt 0) { Write-Host "检测到 $count 条 $ruleId 诊断。" }
    $detections += $count
}
Write-Host "实际检测到 $detections 条析构/索引器相关诊断。"
Write-Host -ForegroundColor Green "SUCCESS: Test '$TestName' 检测到 $detections 条析构/索引器相关诊断。"
